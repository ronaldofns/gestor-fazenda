...
export default function HistoricoAlteracoes({
  ...
  const getDiff = (before: any, after: any) => {
    ...
    allKeys.forEach((key) => {
      ...
      if (
        ["id", "createdAt", "updatedAt", "synced", "remoteId"].includes(key)
      ) {
        return;
      }
  ...
  const handleRestaurar = async (auditId: string) => {
    ...
    if (!audit || !audit.before) {
      showToast({
        ...
        message: "Versão anterior não disponível.",
      });
      return;
    }
    ...
    setConfirmDialog({
      ...
      message:
        "Deseja realmente restaurar esta versão? Os dados atuais serão substituídos.",
      variant: "warning",
      onConfirm: async () => {
        ...
        try {
          ...
          switch (entity) {
            ...
            case "usuario":
              ...
              showToast({
                type: "warning",
                title: "Restauração de usuário",
                message:
                  "Restauração de usuários requer atenção especial. Use a edição manual.",
              });
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <span
                            className={`px-2 py-1 text-xs font-semibold rounded-full ${getActionColor(audit.action)}`}
                          >
                        ...
                        {diff && diff.length > 0 && (
                          <div className="mt-3">
                            <button
                              onClick={() =>
                                setMostrarDiff(
                                  isMostrandoDiff ? null : audit.id,
                                )
                              }
                              className="flex items-center gap-1 text-sm text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300"
                            >
                              <Icons.Eye className="w-4 h-4" />
                              {isMostrandoDiff ? "Ocultar" : "Mostrar"}{" "}
                              alterações ({diff.length})
                            </button>
