...
import { ConfinamentoAnimal, Animal, Confinamento } from '../db/models';
...
import { validarConfinamentoAnimal, validarEntradaAnimal } from '../utils/confinamentoRules';
...
const schema = z.object({
  ...
  pesoEntrada: z.number().min(0.01, 'Peso deve ser maior que zero'),
...
export default function ConfinamentoAnimalModal({
  ...
  const titulo = mode === 'create' ? 'Adicionar Animal ao Confinamento' : 'Editar Vínculo Animal-Confinamento';
  ...
  useEffect(() => {
    if (animalId) {
      db.animais.get(animalId).then(animal => {
        ...
        if (animal && animal.pesoAtual && watch('pesoEntrada') === 0) {
  ...
  const onSubmit = async (values: FormDataConfinamentoAnimal) => {
    ...
    try {
      ...
      if (mode === 'create') {
        const validacaoEntrada = await validarEntradaAnimal(values.animalId, confinamentoId);
        if (!validacaoEntrada.valido) {
          showToast({ type: 'error', message: validacaoEntrada.erro || 'Animal não pode ser adicionado' });
          setIsSubmitting(false);
          return;
        }
      }

      const dataEntradaBanco = converterDataParaFormatoBanco(values.dataEntrada);
      ...
      if (mode === 'edit' && initialData) {
        ...
        const vínculoAtualizado = await db.confinamentoAnimais.get(initialData.id);
        if (vínculoAtualizado) {
          await createSyncEvent('UPDATE', 'confinamentoAnimal', initialData.id, vínculoAtualizado);
        }

        showToast({ type: 'success', message: 'Vínculo atualizado com sucesso!' });
      } else {
        // Criar novo vínculo
        const novoId = uuid();
        ...
        // Criar evento de sincronização
        await createSyncEvent('INSERT', 'confinamentoAnimal', novoId, novoVínculo);

        showToast({ type: 'success', message: 'Animal adicionado ao confinamento com sucesso!' });
      }
      ...
      showToast({ type: 'error', message: error.message || 'Erro ao salvar vínculo' });
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
      ...
      <div>
        ...
        <AnimalSearchCombobox
          ...
          onChange={(value) => setValue('animalId', value, { shouldValidate: true })}
          disabled={mode === 'edit'}
        />
        {errors.animalId && (
