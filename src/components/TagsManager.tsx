...
const TagsManager = memo(function TagsManager({
  ...
  const tags =
    useLiveQuery(() => db.tags.filter((t) => !t.deletedAt).toArray(), []) ?? [];

  const [isOpen, setIsOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<"all" | "popular" | "categories">(
    "all",
  );
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          showToast({ type: "error", title: "Erro ao excluir", message: msg });
        }
      },
    });
  };
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      showToast({
        ...
        message: `${allTags.length} tags exportadas`,
      });
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : "Tente novamente.";
      console.error("Erro ao exportar tags:", error);
      showToast({ type: "error", title: "Erro ao exportar", message: msg });
    }
  };
    ...
    reader.onload = async (e) => {
      try {
        ...
        showToast({
          ...
          message: `${importedCount} tags importadas`,
        });
        event.target.value = "";
      } catch (error: unknown) {
      ...
      onConfirm: async () => {
        try {
          ...
          showToast({
            ...
            message: `${allTags.length} tags removidas`,
          });
        } catch (error: unknown) {
          const msg =
            error instanceof Error ? error.message : "Tente novamente.";
          console.error("Erro ao limpar tags:", error);
          showToast({ type: "error", title: "Erro ao limpar", message: msg });
        } finally {
          setConfirmDialog({
            open: false,
            title: "",
  ...
  const content = (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <label
          title="Importar"
          className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 hover:shadow-md transition-all cursor-pointer"
        >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">
                        Usado {tag.usageCount}×{" "}
                        {tag.synced ? "• Sincronizado" : "• Pendente"}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
          ...
          <div className="space-y-2">
            {popularTags.length === 0 ? (
              ...
              popularTags.map((tag, index) => (
                ...
                >
                  <div className="shrink-0 w-8 h-8 flex items-center justify-center bg-gray-100 dark:bg-slate-800 rounded-full font-bold text-gray-600 dark:text-slate-400">
                    #{index + 1}
                  </div>
                  <div
