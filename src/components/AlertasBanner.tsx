        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-bold text-gray-900 dark:text-white">
              🚨 {totalNaoLidos}{" "}
              {totalNaoLidos === 1 ? "Alerta Não Lido" : "Alertas Não Lidos"}
            </h3>
      ...
      <div className="space-y-2">
        ...
        {alertasNaoLidos.length > 3 && (
          <div className="text-center pt-2">
            ...
            >
              Ver mais {alertasNaoLidos.length - 3}{" "}
              {alertasNaoLidos.length - 3 === 1 ? "alerta" : "alertas"}
...
function AlertaItem({ alerta }: { alerta: Alerta }) {
  const IconComponent =
    Icons[alerta.icone as keyof typeof Icons] || Icons.AlertCircle;
  ...
  const getBgColor = (cor: string) => {
    switch (cor) {
      ...
      case "amber":
        return "bg-amber-100 dark:bg-amber-900/20 border-amber-300 dark:border-amber-800";
      case "orange":
