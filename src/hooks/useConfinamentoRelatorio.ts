...
export function useConfinamentoRelatorio(fazendaId?: string | null) {
  const dados = useLiveQuery(async (): Promise<DadosConfinamentoExportacao | null> => {
    const confinamentos = await db.confinamentos
      .filter((c) => !c.deletedAt && (!fazendaId || c.fazendaId === fazendaId))
      .toArray();
    ...
    for (const conf of confinamentos) {
      ...
      const custoTotal = alimentacao.reduce((s, a) => s + (a.custoTotal ?? 0), 0);
      ...
      if (vinculos.length > 0) {
        pesoMedioEntrada = vinculos.reduce((s, v) => s + (v.pesoEntrada ?? 0), 0) / vinculos.length;
        const pesagensRaw = await db.confinamentoPesagens.toArray();

        for (const v of vinculos) {
          if (v.dataSaida && v.pesoSaida != null) {
            const res = calcularGMD(v.pesoEntrada, v.pesoSaida, v.dataEntrada, v.dataSaida);
            if (res.gmd != null) {
      ...
      const gmdMedio = gmdPorVinculo.length > 0 ? gmdPorVinculo.reduce((a, b) => a + b, 0) / gmdPorVinculo.length : 0;

      const arrobas = kgGanhoTotal / ARROBA_KG;
      arrobasGeral += arrobas;
      const custoPorArroba = arrobas > 0 ? custoTotal / arrobas : null;
      const diasMedio = diasList.length > 0 ? diasList.reduce((a, b) => a + b, 0) / diasList.length : 0;
    ...
    const totalAnimais = porConfinamento.reduce((s, c) => s + c.totalAnimais, 0);
    const gmdMedioGeral = gmdList.length > 0 ? gmdList.reduce((a, b) => a + b, 0) / gmdList.length : 0;
    const custoPorArrobaGeral = arrobasGeral > 0 ? custoTotalGeral / arrobasGeral : null;

    const ativosDerivado = porConfinamento.filter((c) => c.status === 'ativo').length;
    return {
      resumo: {
