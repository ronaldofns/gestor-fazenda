...
export function useAlertas(
  ...
  const confinamentosRaw =
    useLiveQuery(
      () => db.confinamentos.filter((c) => !c.deletedAt).toArray(),
      [],
    ) || [];
  const confinamentoAnimaisRaw =
    useLiveQuery(
      () => db.confinamentoAnimais.filter((v) => v.deletedAt == null).toArray(),
      [],
    ) || [];
  // usar pesagens gerais (por animal)
  // já carregado acima como pesagensRaw
  ...
  const alertas = useMemo<Alerta[]>(() => {
    ...
    const bezerrosSemDesmama = animais.filter((animal) => {
      ...
      const hoje = new Date();
      const mesesIdade =
        (hoje.getTime() - dataNasc.getTime()) / (1000 * 60 * 60 * 24 * 30);
    ...
    const matrizesSet = new Set(
      genealogiasRaw.map((g) => g.matrizId).filter(Boolean),
    );
    const matrizesImprodutivas = animais.filter((animal) => {
      ...
      const ultimoParto = new Date(
        Math.max(...datasNascimento.map((d) => d.getTime())),
      );
      const hoje = new Date();
      const mesesSemParto =
        (hoje.getTime() - ultimoParto.getTime()) / (1000 * 60 * 60 * 24 * 30);
    ...
    const animaisComVacinasVencidas = animais.filter((animal) => {
      const vacinacoesAnimal = vacinacoesRaw
        ...
        .sort((a, b) => {
          const dataA = new Date(
            a.dataAplicacao.includes("/")
              ? a.dataAplicacao.split("/").reverse().join("-")
              : a.dataAplicacao,
          );
          const dataB = new Date(
            b.dataAplicacao.includes("/")
      ...
      const dataRef =
        ultimaVacinacao.dataVencimento || ultimaVacinacao.dataAplicacao;
      ...
      const mesesDesde =
        (hoje.getTime() - dataRefDate.getTime()) / (1000 * 60 * 60 * 24 * 30);
      return mesesDesde > 6;
    });
    ...
    if (!fazendaId) {
      ...
      fazendasRaw.forEach((fazenda) => {
        const animaisFazenda = animaisRaw.filter(
          (a) => a.fazendaId === fazenda.id,
        );
    ...
    const confinamentoIdsAtivos = new Set(
      confinamentosFiltrados
        .filter((c) => {
          const vinculos = confinamentoAnimaisRaw.filter(
            (v) => v.confinamentoId === c.id,
          );
    ...
    // Mapear última pesagem por animal
    ...
    // Mapear última pesagem por vínculo (confinamentoAnimalId)
    // Usa pesagens gerais do animal, mas apenas as com data >= dataEntrada do vínculo
    const ultimaPesagemPorVinculo = new Map<
      string,
      { peso: number; data: string }
    >();
    for (const v of confinamentoAnimaisRaw) {
      const pesagensDoAnimal = pesagensRaw
        .filter((p) => p.animalId === v.animalId)
        .filter((p) => {
          if (!p.dataPesagem) return false;
          return new Date(p.dataPesagem) >= new Date(v.dataEntrada);
        })
        .sort(
          (a, b) =>
      ...
      if (pesagensDoAnimal.length > 0) {
        ultimaPesagemPorVinculo.set(v.id, {
          peso: pesagensDoAnimal[0].peso,
          data: pesagensDoAnimal[0].dataPesagem,
        });
      }
    }

    const animaisComBaixoGMD: Array<{
      id: string;
      brinco?: string;
      nome?: string;
    ...
    for (const v of vinculosAtivos) {
      const ultima = ultimaPesagemPorAnimal.get(v.animalId);
      const pesoAtual =
        ultima?.peso ?? animaisRaw.find((a) => a.id === v.animalId)?.pesoAtual;
    ...
    // ========================================
    const animaisSemPesagemRecente: Array<{
      id: string;
      brinco?: string;
      nome?: string;
      confinamentoNome: string;
    ...
    for (const v of vinculosAtivos) {
      ...
      const diasSemPesagem = Math.floor(
        (hoje.getTime() - dataRef.getTime()) / (1000 * 60 * 60 * 24),
      );
    ...
    // ========================================
    // Mapear todas as pesagens por animalId
    const pesagensPorAnimal = new Map<
      string,
      Array<{ peso: number; data: string }>
    >();
    for (const p of pesagensRaw) {
      const list = pesagensPorAnimal.get(p.animalId) ?? [];
      list.push({ peso: p.peso, data: p.dataPesagem });
      pesagensPorAnimal.set(p.animalId, list);
    }
    const animaisPesoEstagnado: Array<{
      id: string;
      brinco?: string;
      nome?: string;
      confinamentoNome: string;
    ...
    for (const v of vinculosAtivos) {
      const list = (pesagensPorAnimal.get(v.animalId) ?? []).sort(
      ...
      if (
        diasEntre >= DIAS_PESO_ESTAGNADO &&
        deltaPeso <= DELTA_PESO_ESTAGNADO_KG
      ) {
        const animal = animaisRaw.find((a) => a.id === v.animalId);
        const conf = confinamentosRaw.find((c) => c.id === v.confinamentoId);
    ...
    // confinamentoPesagens removed; usamos pesagensRaw
