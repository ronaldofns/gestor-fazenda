...
export async function pullUpdates() {
  ...
  try {
    ...
    if (servPesagens && servPesagens.length > 0) {
      ...
      const idsParaDeletar = todasPesagensLocais
        ...
        .map(p => p.id);
      if (idsParaDeletar.length > 0) await db.pesagens.bulkDelete(idsParaDeletar);
  ...
  try {
    ...
    if (servVacinacoes && servVacinacoes.length > 0) {
      const servUuids = new Set(servVacinacoes.map((v: any) => v.uuid).filter(Boolean));
      ...
      const idsParaDeletar = todasVacinacoesLocais
        ...
        .map(v => v.id);
      if (idsParaDeletar.length > 0) await db.vacinacoes.bulkDelete(idsParaDeletar);
  ...
  try {
    ...
    const n = await pullEntity({
      ...
      mapper: (s: any) => ({
        id: s.uuid,
        fazendaId: remoteIdToFazendaId.get(Number(s.fazenda_id)) ?? remoteIdToFazendaId.get(s.fazenda_id) ?? String(s.fazenda_id ?? ''),
        nome: s.nome,
        dataInicio: s.data_inicio,
        dataFimPrevista: s.data_fim_prevista || undefined,
        dataFimReal: s.data_fim_real || undefined,
        status: s.status,
        precoVendaKg: s.preco_venda_kg != null ? Number(s.preco_venda_kg) : undefined,
    ...
    console.error('Erro ao processar pull de confinamentos:', err?.message ?? err, err);
  ...
  try {
    ...
    const n = await pullEntity({
      ...
      mapper: (s: any) => ({
        id: s.uuid,
        confinamentoId: remoteIdToConfinamentoId.get(Number(s.confinamento_id)) ?? remoteIdToConfinamentoId.get(s.confinamento_id) ?? String(s.confinamento_id ?? ''),
    ...
    console.error('Erro ao processar pull de confinamento_animais:', err?.message ?? err, err);
    endSyncStep('Pull Confinamento Animais', 0);
  }

  const vinculosLocais = await db.confinamentoAnimais.toArray();
  ...
  try {
    ...
    const n = await pullEntity({
      ...
      mapper: (s: any) => ({
        id: s.uuid,
        confinamentoAnimalId: remoteIdToConfinamentoAnimalId.get(Number(s.confinamento_animal_id)) ?? remoteIdToConfinamentoAnimalId.get(s.confinamento_animal_id) ?? String(s.confinamento_animal_id ?? ''),
        data: s.data,
        peso: s.peso,
        observacoes: s.observacoes || undefined,
        createdAt: s.created_at,
    ...
    console.error('Erro ao processar pull de confinamento_pesagens:', err?.message ?? err, err);
  ...
  try {
    ...
    console.error('Erro ao processar pull de confinamento_alimentacao:', err?.message ?? err, err);
      ...
      try {
        ...
        const n = await pullEntity({
          ...
          mapper: (s: any) => ({
            id: s.uuid,
            animalId: s.animal_id ?? '',
            confinamentoAnimalId: s.confinamento_animal_id != null
              ? (remoteIdToConfinamentoAnimalId.get(Number(s.confinamento_animal_id)) ?? remoteIdToConfinamentoAnimalId.get(s.confinamento_animal_id))
              : undefined,
            data: s.data,
            tipo: s.tipo,
  ...
  try {
    ...
    emitSyncProgress('pull', currentStep, totalSteps, 'Sincronizando Notificações lidas...');
    let nNotif = 0;
  ...
  try {
    ...
    if (db.audits) {
      ...
      if (errorAudits) {
        console.error('Erro ao buscar auditoria do servidor:', errorAudits?.message ?? errorAudits);
