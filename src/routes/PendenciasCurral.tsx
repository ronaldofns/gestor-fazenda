...
export default function PendenciasCurral() {
  ...
  const { alertas } = useAlertas(fazendaAtivaId || undefined);
  ...
  const semPesagemRecente = useMemo(() => {
    const statusMap = new Map(
      statusRaw.map((s) => [s.id, s.nome?.toLowerCase() || ""]),
    );
    ...
    return animais.filter((animal) => {
      const pesagensAnimal = pesagensRaw
        ...
        .map((p) => ({
          ...p,
          data: new Date(
            p.dataPesagem.includes("/")
              ? p.dataPesagem.split("/").reverse().join("-")
              : p.dataPesagem,
          ),
        }))
    ...
    <div className="p-2 sm:p-4 md:p-6 max-w-4xl mx-auto w-full overflow-x-hidden">
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">
          O que precisa de atenção no curral. Clique no animal para abrir e
          registrar desmama, pesagem ou vacina.
        </p>
        {totalPendencias === 0 && (
      ...
      <div className="space-y-6">
        {secoes.map((secao) => {
          const Icon =
            Icons[secao.icone as keyof typeof Icons] ?? Icons.AlertCircle;
          ...
          return (
            ...
            >
              <div
                className={`px-4 py-3 border-b ${corClasses.border} flex items-center gap-3 ${corClasses.headerBg}`}
              >
                <div
                  className={`p-2 rounded-lg ${corClasses.bg} border ${corClasses.border}`}
                >
                ...
                <div
                  className={`flex flex-col items-center justify-center rounded-lg border ${corClasses.badgeBorder} ${corClasses.badgeBg} ${corClasses.badgeText} px-3 py-1.5 min-w-[60px]`}
                >
                  <span className="text-lg font-bold leading-none">{num}</span>
                  <span className="text-xs font-medium leading-tight mt-0.5">
                    {num === 1 ? "animal" : "animais"}
                  </span>
                </div>
              </div>
              <ul
                className={`divide-y ${corClasses.border} bg-white dark:bg-slate-800`}
              >
                {secao.itens
                  .slice(0, 50)
                  .map(
                    (item: { id: string; brinco?: string; nome?: string }) => (
                      <li key={item.id}>
                        {podeVerAnimais ? (
                          <Link
              ...
              {secao.itens.length > 50 && (
                <div
                  className={`px-4 py-2 ${corClasses.headerBg} text-sm text-gray-600 dark:text-slate-400 border-t ${corClasses.border}`}
                >
                  Mostrando 50 de {secao.itens.length}. Filtre em Animais para
                  ver todos.
                </div>
              )}
            </section>
