...
const HistoricoAlteracoes = lazy(
  () => import("../components/HistoricoAlteracoes"),
);
...
export default function Animais() {
  ...
  const [filtroMesesNascimento, setFiltroMesesNascimento] = useState<number[]>(
    [],
  ); // Múltipla seleção de meses
  ...
  const parseDataNascimentoLocal = (
  ...
  const timelineDataQuery = useLiveQuery(async () => {
    if (!timelineAnimalId) return null;
    try {
      const animal = await db.animais.get(timelineAnimalId);
      if (!animal) return null;
      const [desmama, pesagens, vacinacoes, vinculos, ocorrencias, raca] =
        await Promise.all([
          ...
          db.confinamentoAnimais
            .where("animalId")
            .equals(timelineAnimalId)
            .toArray(),
          db.ocorrenciaAnimais
            .where("animalId")
            .equals(timelineAnimalId)
            .toArray(),
          animal.racaId
            ? db.racas.get(animal.racaId)
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        menuMesesRef.current &&
        !menuMesesRef.current.contains(event.target as Node)
      ) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        colunasPopoverRef.current &&
        !colunasPopoverRef.current.contains(event.target as Node)
      ) {
  ...
  const genealogias =
    useLiveQuery(
      () => db.genealogias.filter((g) => !g.deletedAt).toArray(),
      [],
    ) || [];
  ...
  const animaisFiltrados = useMemo(() => {
    ...
    if (temBusca) {
      buscaPreparada = {
        fazendaMap: new Map(
          Array.from(fazendaMap.entries()).map(([k, v]) => [
            k,
            v?.toLowerCase() || "",
          ]),
        ),
        tipoMap: new Map(
          Array.from(tipoMap.entries()).map(([k, v]) => [
            k,
            v?.toLowerCase() || "",
          ]),
        ),
        statusMap: new Map(
          Array.from(statusMap.entries()).map(([k, v]) => [
            k,
            v?.toLowerCase() || "",
          ]),
        ),
        racaMap: new Map(
          Array.from(racaMap.entries()).map(([k, v]) => [
            k,
            v?.toLowerCase() || "",
          ]),
        ),
    ...
    for (const animal of animaisRaw) {
      ...
      // Filtro por sexo
      if (filtroSexo && filtroSexo.trim() !== "" && animal.sexo !== filtroSexo)
        continue;
      ...
      if (
        filtroStatusId &&
        filtroStatusId.trim() !== "" &&
        animal.statusId !== filtroStatusId
      )
        continue;
      ...
      if (temFiltroTags) {
        ...
        if (tagFilterMode === "any") {
          if (!selectedTags.some((tagId) => animalTagIds.includes(tagId)))
            continue;
        } else {
          if (!selectedTags.every((tagId) => animalTagIds.includes(tagId)))
            continue;
        }
      }
      ...
      if (temBusca) {
        ...
        const match =
          ...
          (buscaPreparada!.fazendaMap.get(animal.fazendaId) || "").includes(
            termoBusca,
          ) ||
          (buscaPreparada!.tipoMap.get(animal.tipoId) || "").includes(
            termoBusca,
          ) ||
      ...
      filtrados.sort((a, b) => {
        ...
        switch (sortField) {
          ...
          case "nome":
            ...
            break;
          case "tipo":
            comp = (tipoMap.get(a.tipoId) || "").localeCompare(
              tipoMap.get(b.tipoId) || "",
            );
            break;
          case "status":
            comp = (statusMap.get(a.statusId) || "").localeCompare(
              statusMap.get(b.statusId) || "",
            );
            break;
          case "sexo":
          ...
          case "raca":
            ...
            break;
          case "fazenda":
            comp = (fazendaMap.get(a.fazendaId) || "").localeCompare(
              fazendaMap.get(b.fazendaId) || "",
            );
            break;
          case "dataNascimento": {
    ...
    filtroStatusId,
    filtroRacaId,
    filtroSomenteBezerros,
    filtroMesesNascimento,
    filtroAno,
    selectedTags,
    tagFilterMode,
    buscaParaFiltro,
  ...
  const matrizesStats = useMemo(() => {
    ...
    if (matrizesCount.size > 0) {
      ...
      matrizesCount.forEach((_count, matrizId) => {
        const animalMatriz = animaisMap.get(matrizId);
        if (!animalMatriz?.tipoId || !tipoMatrizIds.has(animalMatriz.tipoId))
          return;
  ...
  const bezerrosStats = useMemo(() => {
    const bezerros = animaisFiltrados.filter(
      (animal) => animal.tipoId && tipoBezerroIds.has(animal.tipoId),
    );
  ...
  const racasStats = useMemo(() => {
    const racasCounts = new Map<
      string,
      { nome: string; total: number; machos: number; femeas: number }
    >();

    animaisFiltrados.forEach((animal) => {
      const racaNome = animal.racaId
        ? racaMap.get(animal.racaId) || "Sem raça"
        : "Sem raça";
      const current = racasCounts.get(racaNome) || {
        nome: racaNome,
        total: 0,
        machos: 0,
        femeas: 0,
    ...
    return Array.from(racasCounts.values()).sort((a, b) => b.total - a.total); // Ordenar por total decrescente
  ...
  return (
    <div className="p-2 sm:p-3 md:p-4 text-gray-900 dark:text-slate-100 relative min-w-0 max-w-full overflow-x-hidden">
      ...
      <div
        className={`${getPrimaryCardClass(primaryColor)} rounded-xl shadow-sm mb-4 sm:mb-6 p-3 sm:p-4 md:p-6 min-w-0 max-w-full overflow-hidden`}
      >
        {/* HEADER */}
        <div className="flex flex-row items-start justify-between gap-2 mb-6">
          <div className="flex-1 min-w-0">
            <h1
              className={`text-2xl font-bold ${getTitleTextClass(primaryColor)}`}
            >
              Gestão de Animais
            </h1>
                    ...
                    options={[
                      ...
                      ...(() => {
                        ...
                        animaisRaw
                          .filter((a) => !a.deletedAt && a.dataNascimento)
                          .forEach((a) => {
                            const data = parseDataNascimentoLocal(
                              a.dataNascimento!,
                            );
                            if (data) anos.add(data.getFullYear());
                          });
                    ...
                    >
                      <span className="truncate">
                        {filtroMesesNascimento.length === 0
                          ? "Todos os meses"
                          : filtroMesesNascimento.length === 1
                            ? nomeMes(filtroMesesNascimento[0])
                            : `${filtroMesesNascimento.length} meses`}
                      </span>
                      <Icons.ChevronDown
                        className={`w-4 h-4 transition-transform ${menuMesesAberto ? "rotate-180" : ""}`}
                      />
                    </button>
                    {menuMesesAberto && (
                      <div className="absolute z-[100] w-full left-0 mt-1 bg-white dark:bg-slate-900 rounded-md shadow-lg border border-gray-200 dark:border-slate-700 max-h-64 overflow-y-auto">
                        ...
                        <div className="py-1">
                          ...
                          {Array.from({ length: 12 }, (_, i) => {
                            const mes = i + 1;
                            const isSelected =
                              filtroMesesNascimento.includes(mes);
                            return (
                              <label
                                key={mes}
                              ...
                              >
                                <input
                                  ...
                                  onChange={(e) => {
                                    if (e.target.checked)
                                      setFiltroMesesNascimento(
                                        [...filtroMesesNascimento, mes].sort(
                                          (a, b) => a - b,
                                        ),
                                      );
                                    else
                                      setFiltroMesesNascimento(
                                        filtroMesesNascimento.filter(
                                          (m) => m !== mes,
                                        ),
                                      );
                                  }}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                <div className="flex flex-wrap gap-2">
                  ...
                  {filtroMesesNascimento.length > 0 && (
                    <span className="px-2 py-1 text-xs rounded-md bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">
                      Meses:{" "}
                      {filtroMesesNascimento.map((m) => nomeMes(m)).join(", ")}
                    </span>
                  )}
                  {filtroAno && (
                    <span className="px-2 py-1 text-xs rounded-md bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300">
        ...
        {/* KPIs */}
        <div
          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 ${filtrosAbertos ? "mb-6" : "mb-2"}`}
        >
          {/* Matrizes */}
          <div className="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="text-3xl font-bold text-purple-600 dark:text-purple-400">
                {Array.from(matrizesStats.values()).reduce(
                  (sum, count) => sum + count,
                  0,
                )}
              </div>
            ...
            <div className="flex flex-col gap-2 pt-2 border-t border-gray-200 dark:border-slate-700">
              {Array.from(matrizesStats.entries()).length > 0 ? (
                ...
                <div className="text-sm text-gray-400 dark:text-slate-500 italic">
                  ...
                  <div className="absolute right-0 top-full mt-1 z-[100] w-56 py-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-lg shadow-lg">
                    ...
                    <div className="px-3 py-2 border-t border-gray-200 dark:border-slate-600">
                      <button
                        type="button"
                        onClick={() =>
                          setVisibleColumns({ ...VISIBLE_COLUMNS_DEFAULT })
                        }
          ...
          <div className="md:hidden space-y-3 mb-4">
            {animaisPagina.length === 0 ? (
              ...
              animaisPagina.map((animal) => {
                const genealogia = genealogiaMap.get(animal.id);
                const matrizId = genealogia?.matrizId || animal.matrizId;
                const matrizAnimal = matrizId
                  ? animaisMap.get(matrizId)
                  : undefined;
                const matrizIdentificador = matrizAnimal?.brinco || "";
                const matrizNome = matrizAnimal?.nome || "";
                const tipoMatrizId =
                  genealogia?.tipoMatrizId || matrizAnimal?.tipoId;
                const tipoMatrizNome = tipoMatrizId
                  ? tipoMap.get(tipoMatrizId)
                  : null;
                    ...
                    <div className="flex-1 items-start justify-between mb-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center gap-2 mb-1">
                          ...
                          {isMatriz && (
                            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border border-purple-500 bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 shrink-0">
                              Matriz
                            </span>
                          )}
                          {isBezerro && !isMatriz && (
                            <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border border-green-500 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 shrink-0">
                              Bezerro
                            </span>
                          )}
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      ...
                      {visibleColumns.sexo && (
                        <div>
                          ...
                          <span
                            className={`ml-1 font-medium ${animal.sexo === "M" ? "text-blue-600 dark:text-blue-400" : "text-pink-600 dark:text-pink-400"}`}
                          >
                      ...
                      {visibleColumns.matriz && matrizIdentificador && (
                        <div className="col-span-2">
                          ...
                          {matrizNome && (
                            <span className="ml-1 text-xs border border-pink-400 rounded-full text-center px-2 py-0.5 text-gray-500 dark:text-slate-400">
                              {matrizNome}
                            </span>
                          )}
                        </div>
                    ...
                    <div className="flex items-center flex-shrink-0 justify-between border border-gray-200 dark:border-slate-700 rounded mt-4 px-2 py-2">
                      {podeEditarAnimal && (
                        <button
                          ...
                          className={`p-1.5 text-green-400 hover:text-green-700 rounded transition-colors`}
                          title="Editar animal"
                        >
                      ...
                      {podeExcluirAnimal && (
                        <button
                          ...
                          className={`p-1.5 text-red-400 hover:text-red-700 rounded transition-colors`}
                          title="Excluir animal"
                        >
                      ...
                      <button
                        onClick={() => {
                          setHistoricoEntityId(animal.id);
                          setHistoricoOpen(true);
                        }}
                        className={`p-1.5 text-blue-400 hover:text-blue-700 rounded transition-colors`}
                        title="Ver histórico"
                      >
                      ...
                      <button
                        onClick={() => {
                          setTimelineAnimalId(animal.id);
                          setTimelineOpen(true);
                        }}
                        className={`p-1.5 text-purple-400 hover:text-purple-700 rounded transition-colors`}
                      ...
                      <button
                        ...
                        disabled={!matrizId}
                        className={`p-1.5 text-yellow-400 hover:text-yellow-700 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed`}
              ...
              <>
                ...
                >
                  {visibleColumns.brinco && (
                    <div
                      className="px-2 sm:px-3 py-1.5 text-left text-xs font-medium text-gray-500 dark:text-slate-300 uppercase cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-700 whitespace-nowrap"
                  ...
                  {visibleColumns.matriz && (
                    <div className="px-2 sm:px-3 py-1.5 text-left text-xs font-medium text-gray-500 dark:text-slate-300 uppercase whitespace-nowrap">
                      Matriz
                    </div>
                  )}
                  ...
                  {visibleColumns.acoes && (
                    <div className="px-2 sm:px-3 py-1.5 text-center text-xs font-medium text-gray-500 dark:text-slate-300 uppercase whitespace-nowrap min-w-[150px]">
                      Ações
                    </div>
                  )}
                </div>
                <List
                  ...
                  rowComponent={({ index, style }) => {
                    ...
                    return (
                      ...
                      >
                        {visibleColumns.brinco && (
                          <div className="px-2 sm:px-3 py-1.5 text-sm font-medium text-gray-900 dark:text-slate-100 min-w-0">
                            <div className="flex items-center justify-between gap-2 min-w-0">
                              <span className="truncate">{animal.brinco}</span>
                              {isMatriz && (
                                <span
                                  className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border border-purple-500 bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 shrink-0"
                                  title="Matriz"
                                >
                                  Matriz
                                </span>
                              ...
                              {isBezerro && !isMatriz && (
                                <span
                                  className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border border-green-500 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 shrink-0"
                                  title="Bezerro"
                                >
                                  Bezerro
                                </span>
                        ...
                        {visibleColumns.nome && (
                          <div
                            className="px-2 sm:px-3 py-1.5 text-sm text-gray-700 dark:text-slate-200 truncate"
                            title={animal.nome || undefined}
                          >
                            {animal.nome || "-"}
                          </div>
                        )}
                        {visibleColumns.tipo && (
                          <div className="px-2 sm:px-3 py-1.5 text-sm text-gray-700 dark:text-slate-200 whitespace-nowrap">
                            {animal.tipoId ? tipoMap.get(animal.tipoId) : "-"}
                          </div>
                        )}
                        {visibleColumns.status && (
                        ...
                        {visibleColumns.matriz && (
                          <div className="px-2 sm:px-3 py-1.5 text-sm text-gray-700 dark:text-slate-200">
                            {matrizIdentificador ? (
                              <div className="min-w-0">
                                ...
                                {matrizNome?.trim() && (
                                  <div className="text-xs border border-pink-400 rounded-full text-center py-0.5 text-gray-500 dark:text-slate-400 truncate">
                                    {matrizNome}
                                  </div>
                                )}
                              </div>
                        ...
                        {visibleColumns.tipoMatriz && (
                          <div className="px-2 sm:px-3 py-1.5 text-sm whitespace-nowrap">
                            {tipoMatrizNome ? (
                        ...
                        {visibleColumns.sexo && (
                          <div className="px-2 sm:px-3 py-1.5 text-sm">
                            <span
                              className={
                                animal.sexo === "M"
                        ...
                        {visibleColumns.fazenda && (
                          <div
                            className="px-2 sm:px-3 py-1.5 text-sm text-gray-700 dark:text-slate-200 truncate"
                            title={fazendaMap.get(animal.fazendaId)}
                          >
                            {fazendaMap.get(animal.fazendaId) || "-"}
                          </div>
                        )}
                        {visibleColumns.tags && (
                          <div className="px-2 sm:px-3 py-1.5 text-sm min-w-0">
                            <AnimalTags tags={animalTagsMap.get(animal.id)} />
                          </div>
                        )}
                        {visibleColumns.acoes && (
                          <div className="px-2 sm:px-3 py-1.5 text-center min-w-[150px]">
                            <div className="flex items-center justify-center gap-2 flex-nowrap shrink-0">
                              {podeEditarAnimal && (
                                <button
                                  type="button"
                                  onClick={() => handleEditarAnimal(animal)}
                                  className={`p-1 shrink-0 text-green-400 hover:text-green-600 rounded transition-colors`}
                                  title="Editar animal"
                                >
                              ...
                              {podeExcluirAnimal && (
                                <button
                                  type="button"
                                  onClick={() => handleExcluirAnimal(animal)}
                                  className={`p-1 shrink-0 text-red-400 hover:text-red-600 rounded transition-colors`}
                                  title="Excluir animal"
                                >
                                  <Icons.Trash2 className="w-5 h-5 sm:w-4 sm:h-4" />
                                </button>
                              )}
                              <button
                                type="button"
                                ...
                                className={`p-1 shrink-0 text-blue-400 hover:text-blue-600 rounded transition-colors`}
                                title="Ver histórico"
                              >
                              ...
                              <button
                                ...
                                className={`p-1 shrink-0 text-purple-400 hover:text-purple-600 rounded transition-colors`}
                                title="Linha do tempo"
                              >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (matrizId) {
                                    setArvoreAnimalId(
                                      matrizAnimal?.id || matrizId,
                                    );
                                    setArvoreOpen(true);
                                  }
                                }}
                                disabled={!matrizId}
                                className={`p-1 shrink-0 text-yellow-400 hover:text-yellow-600 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed`}
                                title="Ver árvore genealógica"
                              >
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
                <thead className="sticky top-0 z-10 bg-gray-100 dark:bg-slate-800">
                  <tr>
                    ...
                    {visibleColumns.matriz && (
                      <th className="px-2 sm:px-3 py-1.5 text-left text-xs font-medium text-gray-500 dark:text-slate-300 uppercase whitespace-nowrap">
                        Matriz
                      </th>
                    )}
                    {visibleColumns.tipoMatriz && (
                    ...
                    {visibleColumns.acoes && (
                      <th
                        className="sticky right-0 z-10 px-2 sm:px-3 py-1.5 text-center text-xs font-medium text-gray-500 min-w-[150px] w-[150px]
                  ...
                  {animaisPagina.length === 0 ? (
                    <tr>
                      <td
                        colSpan={Math.max(
                          1,
                          COLUNAS_TABELA.filter((c) => visibleColumns[c.id])
                            .length,
                        )}
                    ...
                    animaisPagina.map((animal) => {
                      ...
                      return (
                        <tr
                          key={animal.id}
                          className="odd:bg-white even:bg-gray-50 dark:odd:bg-slate-900 dark:even:bg-slate-800 hover:bg-gray-100 dark:hover:bg-slate-700"
                        >
                          {visibleColumns.brinco && (
                          ...
                          {visibleColumns.nome && (
                            <td
                              className="px-2 sm:px-3 py-1.5 text-sm text-gray-700 dark:text-slate-200 sm:max-w-none truncate"
                              title={animal.nome || undefined}
                            >
                              {animal.nome || "-"}
                            </td>
                          ...
                          {visibleColumns.tipo && (
                            <td className="px-2 sm:px-3 py-1.5 whitespace-nowrap text-sm text-gray-700 dark:text-slate-200">
                              {animal.tipoId ? tipoMap.get(animal.tipoId) : "-"}
                            </td>
                          )}
                          {visibleColumns.status && (
                            ...
                            <td className="px-2 sm:px-3 py-1.5 text-sm text-gray-700 dark:text-slate-200">
                              {matrizIdentificador ? (
                                <div className="min-w-0">
                                  ...
                                  {matrizNome?.trim() && (
                                    <div className="text-xs text-gray-500 border border-pink-400 rounded-full text-center py-0.5 dark:text-slate-400 truncate">
                                      {matrizNome}
                                    </div>
                                  )}
                                </div>
                          ...
                          {visibleColumns.fazenda && (
                            <td
                              className="px-2 sm:px-3 py-1.5 whitespace-nowrap text-sm text-gray-700 dark:text-slate-200 sm:max-w-none truncate"
                              title={fazendaMap.get(animal.fazendaId)}
                            >
                              {fazendaMap.get(animal.fazendaId) || "-"}
                            </td>
                          ...
                          {visibleColumns.tags && (
                            <td className="px-2 sm:px-3 py-1.5 text-sm min-w-0 align-middle">
                              <AnimalTags tags={animalTagsMap.get(animal.id)} />
                            </td>
                          )}
                          {visibleColumns.acoes && (
                              ...
                              <div className="flex items-center justify-center gap-2 flex-nowrap">
                                {podeEditarAnimal && (
                                  <button
                                    ...
                                    title="Editar animal"
                                  >
                                    <Icons.Edit className="w-3.5 h-3.5 sm:w-6 sm:h-6" />
                                  </button>
                                )}
                                {podeExcluirAnimal && (
                                  <button
                                    onClick={() => handleExcluirAnimal(animal)}
                                    className={`p-1 shrink-0 text-red-500 hover:text-red-700 rounded transition-colors`}
                                    title="Excluir animal"
                                  >
                                    <Icons.Trash2 className="w-3.5 h-3.5 sm:w-6 sm:h-6" />
                                  </button>
                                )}
                                <button
                                  onClick={() => {
                                  ...
                                  className={`p-1 shrink-0 text-blue-500 hover:text-blue-700 rounded transition-colors`}
                                  title="Ver histórico"
                                >
                                  <Icons.History className="w-3.5 h-3.5 sm:w-6 sm:h-6" />
                                </button>
                                <button
                                  onClick={() => {
                                    setTimelineAnimalId(animal.id);
                                  ...
                                  className={`p-1 shrink-0 text-purple-500 hover:text-purple-700 rounded transition-colors`}
                                  title="Linha do tempo"
                                >
                                  <Icons.Calendar className="w-3.5 h-3.5 sm:w-6 sm:h-6" />
                                </button>
                                <button
                                  onClick={() => {
                                    if (matrizId) {
                                  ...
                                  disabled={!matrizId}
                                  className={`p-1 shrink-0 text-yellow-500 hover:text-yellow-700 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed`}
                                  title="Ver árvore genealógica"
                                >
                                  <Icons.GitBranch className="w-3.5 h-3.5 sm:w-6 sm:h-6" />
                                </button>
                              </div>
                            </td>
                          )}
      ...
      {historicoOpen && historicoEntityId && (
        <Suspense fallback={null}>
          <HistoricoAlteracoes
            ...
            entityNome={
              animaisFiltrados.find((a) => a.id === historicoEntityId)?.brinco
            }
            onClose={() => {
              ...
              <div>
                ...
                {timelineData?.animal?.brinco && (
                  <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">
                    Brinco {timelineData.animal.brinco}
                  </p>
                )}
              </div>
              <button
